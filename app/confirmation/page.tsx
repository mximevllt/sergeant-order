...
function durationLabel(halfDays: number, packageCode?: unknown) {
  if (packageCode === "TWO_HOURS") return "Forfait 2 heures";
  if (packageCode === "HALF_DAY") return "Demi-journée";
  if (packageCode === "FULL_DAY") return "Journée complète";
  if (packageCode === "TWO_DAYS") return "Deux journées";
...
export default async function ConfirmationPage({ searchParams }: { searchParams: Promise<{ devis?: string }> }) {
  ...
  const paymentPath = `/paiement?devis=${encodeURIComponent(quote.id)}`;
  const paymentHref = actor?.sessionKind === "CUSTOMER" ? paymentPath : `/connexion?returnTo=${encodeURIComponent(paymentPath)}`;

  return <main className="confirmation-page"><header><Link href="/" aria-label="Sergeant Paysage, accueil"><Image src="/logo-sergeant-paysage-blanc.png" alt="Sergeant Paysage" width={1784} height={387} priority /></Link></header><section><div className="confirm-check">✓</div><p>Devis {quote.publicReference}</p><h1>{hold ? "Votre créneau est bloqué." : "Votre devis est enregistré."}</h1><h2>{hold ? `Cette disponibilité vous est réservée jusqu’au ${holdExpires}, le temps d’enregistrer votre garantie bancaire.` : `Son tarif est conservé jusqu’au ${expires}. Vous pouvez le modifier ou le reprendre plus tard.`}</h2><div className="confirm-card"><div><span>Adresse</span><strong>{address}</strong></div><div><span>Prestations</span><strong>{quote.tasks.map(({ label }) => label).join(" + ")}<br />{durationLabel(quote.selectedHalfDays, request.packageCode)}<br /><small>Déplacement inclus dans le forfait</small></strong></div><div><span>{hold ? "Créneau bloqué" : "Souhait indiqué"}</span><strong>{schedule}<br /><small>{hold ? hold.completionLabel : "sous réserve des disponibilités réelles"}</small></strong></div><div><span>Prix du devis</span><strong>{price} TTC<br /><small>débit après la prestation</small></strong></div></div><div className="timeline"><div className="done"><i>✓</i><span>Devis enregistré</span></div><div className={hold ? "done" : ""}><i>{hold ? "✓" : "2"}</i><span>{hold ? "Créneau bloqué" : "Créneau à confirmer"}</span></div><div><i>3</i><span>Carte sécurisée</span></div><div><i>4</i><span>Commande confirmée</span></div></div><div className="confirm-actions">{hold && <Link className="button button-primary" href={paymentHref}>{actor?.sessionKind === "CUSTOMER" ? "Garantir et confirmer" : "Se connecter pour confirmer"}</Link>}<Link href={`/reserver?devis=${encodeURIComponent(quote.id)}`}>{hold ? "Modifier le devis et le créneau" : "Modifier le devis"}</Link><Link href="/espace-client">Voir mes devis</Link><Link href="/">Retour à l’accueil</Link></div><aside><strong>{hold ? "Aucun débit à la réservation." : "Ce devis n’est pas encore une réservation."}</strong><p>{hold ? "Stripe enregistre le moyen de paiement sans communiquer les données de carte au site. Le montant prévu ne sera débité qu’après la prestation." : "Aucun créneau n’est bloqué et aucun paiement n’a été enregistré."}</p></aside></section></main>;
}
