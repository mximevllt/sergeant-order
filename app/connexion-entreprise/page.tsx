...
export default async function StaffLoginPage({ searchParams }: { searchParams: Promise<{ returnTo?: string; erreur?: string }> }) {
  ...
  const testAccess = process.env.TEST_ADMIN_BYPASS === "enabled";
  return <main className="login-page staff-login-page"><section className="login-brand-panel"><Link href="/" aria-label="Sergeant Paysage, accueil"><Image src="/logo-sergeant-paysage-blanc.png" alt="Sergeant Paysage" width={1784} height={387} priority /></Link><div><p>Espace entreprise</p><h2>Chaque personne voit uniquement ce dont elle a besoin.</h2><ul><li>Compte nominatif sur invitation</li><li>Droits contrôlés côté serveur</li><li>Session entreprise limitée à 8 heures</li></ul></div></section><section className="login-card-wrap"><div className="login-card"><p className="kicker">Accès réservé à l’équipe</p><h1>{testAccess ? "Accès administration de test." : "Connectez-vous à votre poste."}</h1><p>{testAccess ? "Saisissez l’une des deux adresses autorisées pour ouvrir directement l’administration. Ce mode doit être retiré avant toute mise en service réelle." : "Utilisez l’adresse professionnelle invitée par l’administrateur. La réponse reste identique même lorsqu’une adresse n’est pas autorisée."}</p>{errorMessage && <p className="login-banner" role="alert">{errorMessage}</p>}<StaffLoginForm returnTo={returnTo} testAccess={testAccess} /></div></section></main>;
}
